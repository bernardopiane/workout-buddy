...
class HomePage extends StatelessWidget {
        ...
        Row(
          ...
          children: [
            Selector<WorkoutPlanManager, int>(
              selector: (_, manager) => manager.selectedPlan?.workoutDays
                  .map((day) => day.workouts.length)
                  .reduce((a, b) => a + b) ?? 0,
              builder: (context, totalWorkouts, child) {
                return _buildStatCard(title: 'Total Workouts', value: '$totalWorkouts');
              },
            ),
  ...
  Widget _buildNextWorkoutsSection(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text("Next Workouts", style: Theme.of(context).textTheme.headlineSmall),
        const SizedBox(height: 16.0),
        Consumer<WorkoutPlanManager>(
          builder: (context, workoutPlanManager, child) {
            ...
            var nextWorkoutDay = workoutPlanManager.selectedPlan!.workoutDays.firstWhere(
            ...
            var muscles = nextWorkoutDay.workouts.map((workout) => workout.images![0]).toSet().toList();
              ...
              children: todayWorkouts.map((day) {
                return GestureDetector(
                  onTap: () => Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => WorkoutDetailsPage(workoutPlan: workoutPlanManager.selectedPlan!),
                    ),
                  ),
                  child: Card(
                    child: SizedBox(
  ...
  Widget _buildCalorieCard({required double caloriesBurned}) {
    return Column(
      children: [
        ...
        const SizedBox(height: 16.0),
        Text(caloriesBurned.toStringAsFixed(2), style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
      ],
    );
  }
    ...
    return Column(
      children: buttons.map((button) {
        return ElevatedButton(
          onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => button['page'] as Widget)),
